...
class MyQuotesStepTwo extends Component {
    ...
    async _updateJobStatus(status){
        ...
        let response = await fetch(`http://${Config.apiServer[Config.server]}/api/omc/job/updateJobStatus/${this.props.jobId}`, options)
    ...
    propmtAcceptJob(){
        Alert.alert(
            ``,
            `Are you confirm to accept this booking ?`,
            [
                {
                    text:'No'
                },
    ...
    propmtCancelJob(){
        Alert.alert(
            ``,
            `Are you confirm to cancel this booking ?`,
            [
                {
                    text:'No'
                },
            ...
            return (
                <View style={styles.symbolBar}>
                    <View style={styles.viewButton}>
                        <Button style={{backgroundColor:'#ED1D2A'}} textStyle={{color:'#fff'}} onPress={()=>this.propmtCancelJob()}>
                                <Icon name="close" iconSet={'FontAwesome'} style={{
                                        color: '#FFF',
                                        fontSize: 20
                                }} />
                                <Text>Cancel Job</Text>
                        </Button>
                        <Button style={{backgroundColor:'#F9D623'}} textStyle={{color:'#fff'}} onPress={()=>this.propmtAcceptJob()}> 
                            <Icon name="calendar-check-o" iconSet={'FontAwesome'} style={{
                                    color: '#FFF',  
                                    fontSize: 20
                            }} />
                            <Text>BOOK NOW</Text>
